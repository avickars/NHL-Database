...
def create_gim_model_sequences():
    # Opening connection
    ...
    mostRecentRun = pd.read_sql_query("select date from script_execution where script = 'create_gim_model_sequences' order by date desc limit 1",
                                      connection)
...
#                                           gameType in ('R','P'  and
    ...
    if len(sequenceNum) == 0:
        ...
        sequenceNum = sequenceNum['sequenceNum'].values[0] + 1
    ...
    sequenceData = create_sequence_data(rawDataFiltered, sequenceNum)
...
def create_sequence_data(rawDataFiltered, sequenceNumberStart):
    ...
    sequenceNum = sequenceNumberStart
    eventNum = 0
    penaltyQueue = Queue()
    gameID = 0
