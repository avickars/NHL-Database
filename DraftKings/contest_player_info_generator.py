...
def get_player_info_api(cursor, connection):
    ...
    for index, contest in contests.iterrows():
        if contest['draftGroupId'] not in draftGroupIDs:
            ...
            for competition in url_data['competitions']:
                contestStartTime = parser.parse(competition['startTime'])
                cursor.execute(f"insert into draft_kings.contest_game_times values ({contest['draftGroupId']},"
                               f"\"{competition['homeTeam']['abbreviation']}\","
                               f"\"{competition['awayTeam']['abbreviation']}\","
                               f"\'{contestStartTime.strftime(date_format)}\');")
                draftGroupIDs.append(contest['draftGroupId'])
...
def main():
    ...
    get_player_info_api(cursor, connection)
    connection.commit()
