...
def get_contests(cursor):
    ...
    for contest in contests:
        ...
        try:
            ...
            try:
                cursor.execute(f"update draft_kings.contests set IsStarred={IsStarred}, "
                           f"gameType={gameType},"
                           f"freeWithCrowns={freeWithCrowns},"
                           f"isBonusFinalized={isBonusFinalized},"
                           f"isSnakeDraft={isSnakeDraft},"
                           f"crownAmount={crownAmount},"
                           f"IsWinnerTakeAll={IsWinnerTakeAll},"
                           f"date=\'{date.today()}\' where"
                           f" id={id}")
            except Errors.ProgrammingError:
                print(f"update draft_kings.contests set IsStarred={IsStarred}, "
                           f"gameType={gameType},"
                           f"freeWithCrowns={freeWithCrowns},"
                           f"isBonusFinalized={isBonusFinalized},"
                           f"isSnakeDraft={isSnakeDraft},"
